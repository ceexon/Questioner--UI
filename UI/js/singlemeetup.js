...
let singleMeetupFetch = () => {
  fetch(`https://questioner--api.herokuapp.com/api/v2/meetups/${meetupId}`, {
    ...
    .then(data => {
      ...
      if (data.status === 404 || data.status === 400 || window.location.href.length === 1) {
        mainBody.innerHTML = "";
        let errorFour04 = document.createElement("div")
        errorFour04.setAttribute("style", "width: 100vw; height: 100vh; z-index: 6; background-color: #fff; font-size: 5em; color: #0d59a0; padding: 10% 0; position: fixed; top:0; bottom:0; left: 0; right: 0; text-align:center")
      ...
      if (data.status === 200) {
        ...
        let meetupDescription = document.querySelector(".meetup-description .meetup-text")
        ...
        let theHappenTime = Date(meetupData.happen_on)
        ...
        let displayTime = firstTimeSection + " " + hourTime + minutesTime + "HRS";
